...
public class treductJayawithdimArray {
...
// big positive number for minimum
    ...
    public static String Bounds(int index_value) {
        ...
        for (int i = 0; i < dim; i++) {
            System.out.print(" " + x_single[i]);
            list.add(x_single[i]);
        }
        List result = list.stream().filter(s -> s > 15).collect(Collectors.toList());
        List result1 = list.stream().filter(s -> s < 0).collect(Collectors.toList());
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int iteration = 1; iteration < 200; iteration++) {
            ...
            {
                Random r = new Random();
                //System.out.println(" single value print ");
                for (int j = 0; j < dim; j++) {
                    double r1 = r.nextDouble();
                    double r2 = r.nextDouble();
                    //System.out.println(" r1 " + r1 + " r2 " + r2);
                    x_single[j] = (int)Math.round(x[i][j] + (r1 * (find_best_value() - x[i][j])) - (r2 * (x[i][j] - find_poor_value())));
        ...
        System.out.println(" fitness after all iterations");

        for (int i = 0; i < population_size; i++) {
            System.out.print(" " + fitness_seq_count[i]);
        }

        int best_solution = find_best_index();
        System.out.println(" best fitness index " + best_solution);

        //System.out.println(" priolity list " + priority_list_for_seq);
        ...
        System.out.println(" prio list for seq " + priority_list_for_seq);
        ...
        double percent_saving = (double) (100 * (16 - objective_value(seq)) / 16);
