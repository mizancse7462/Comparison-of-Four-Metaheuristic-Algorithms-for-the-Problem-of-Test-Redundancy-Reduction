...
public class SineCosineTreduct {
...
// teacher is the best solution in the population
    public static double find_best_value() {
        ...
        double best_value = fitness_seq_count[0];
        for (int i = 0; i < population_size; i++) {

            if (best_value > fitness_seq_count[i]) {

                best_value = fitness_seq_count[i];
                index = i;
        ...
        for (int tt = 0; tt < output_limit; tt++) {
            ...
            for (int iteration = 1; iteration < max_iter; iteration++) {
                ...
                for (int i = 0; i < population_size; i++) // iterate the population
                {
                    ...
                    if (r4 < 0.5) {
                        for (int j = 0; j < dim; j++) {
                            x_single[j] = (int) Math.abs(Math.round(x[i][j] + (r1 * Math.sin(180 * r2 / Math.PI) * Math.abs((r3 * Pbest) - x[i][j]))));
                        }
                        String seq = Bounds(i);
                        //System.out.println(" ");
                        //System.out.println(" seq send " + seq);
                        int x_single_local[]=new int[dim];
                        ...
                        for (int j = 0; i < dim; i++) {
                            x_single[j] = (int) Math.abs(Math.round(x[i][j] + (r1 * Math.cos(180 * r2 / Math.PI) * Math.abs((r3 * Pbest) - x[i][j]))));
                        }
            ...
            // System.out.println(" priority list for seq " + priority_list_for_seq);
